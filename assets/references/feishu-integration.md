...
# 创建项目主文档，使用命名约定
feishu_create_doc --wiki_space "<space_id>" --title "[项目名称] - 项目信息" --content "..."
```

**步骤 3：创建各阶段文档（使用命名约定）**
```bash
# Plan 阶段文档
feishu_create_doc --wiki_space "<space_id>" --title "[项目名称] - Plan - 问题分析" --content "..."

# Do 阶段文档
feishu_create_doc --wiki_space "<space_id>" --title "[项目名称] - Do - 执行日志" --content "..."
```
...
**⚠️ 注意**：飞书 Wiki API 不支持程序化创建文件夹结构。推荐使用命名约定来组织文档：`[项目名称] - <类别> - <文档名>`。
