...
**⚠️ 核心要点**：使用 `feishu_wiki_space_node.create` 创建文件夹节点，然后使用 `feishu_create_doc` + `wiki_node` 在文件夹下创建文档。

**步骤 1：获取知识空间根节点 token**
```bash
# 获取知识空间信息
...
### 创建项目时
使用 `feishu_wiki_space_node.create` 创建文件夹，然后使用 `feishu_create_doc` + `wiki_node` 在文件夹下创建文档：
- 项目索引：使用 `wiki_space` 参数在知识空间根目录创建
- 项目文档：使用命名约定 `[项目名称] - <类别> - <文档名>` 组织

**⚠️ 重要限制**：飞书 Wiki API 不支持程序化创建文件夹结构。使用命名约定确保文档在列表中按项目分组。
