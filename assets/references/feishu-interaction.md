...
## 当前可用方案（飞书 Channel 网关模式）

**适用平台**：OpenClaw 飞书 channel、Hermes Agent 飞书适配器

在飞书 channel 网关模式下（通过飞书机器人与用户交互）：

| 功能 | 状态 | 说明 |
|------|------|------|
| **P2 富文本编号列表** | ✅ **推荐使用** | Agent 通过富文本发送选项，用户回复编号 |
| `clarify` 工具 | ❌ 不可用 | 网关模式无法弹出原生选择框 |
| Interactive Card | ⚠️ 未来支持 | Agent 层无法直接调用卡片发送 API |
...
以下卡片模板供未来平台支持 Interactive Card 时使用。
...
### 当前限制（OpenClaw / Hermes Agent 飞书 Channel）

| 限制 | 说明 |
|------|------|
| **`clarify` 工具不可用** | 网关模式下 `clarify` 工具无法弹出原生选择框 |
| **Agent 层无法直接发送 Interactive Card** | 当前平台适配器不直接暴露卡片 API 给 Agent |
| **卡片回调是无状态的** | 回调消息不携带原始问题的上下文，需要在 `value` 中编码足够信息 |

> ⚠️ **重要**：虽然飞书平台技术层面支持 Interactive Card，但当前 OpenClaw 和 Hermes Agent 的 Agent 层均无法直接调用卡片发送 API。本指南的模板供未来集成使用。当前阶段，PDCA 在飞书上的交互统一使用 **P2 富文本编号列表**。
