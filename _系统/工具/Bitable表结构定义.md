...
| 巡检频率 | 文本 | 1 | Cron 表达式 | 如 `"0 8 * * *"` | 是 |
| 频率来源 | 单选 | 3 | 频率设定来源 | 系统推荐/用户自定义 | 是 |
| Wiki链接 | 超链接 | 15 | 项目文档链接 | `{link: "url", text: "标题"}` | 是 |
...
| 字段名 | 类型 | type值 | 说明 | 值格式 | 必填 |
|--------|------|--------|------|--------|------|
| 任务ID | 文本 | 1 | 唯一标识符 | UUID 字符串 | 是 |
| 项目ID | 文本 | 1 | 关联项目 | 关联到项目主表.项目ID | 是 |
| 任务标题 | 文本 | 1 | 任务描述 | 字符串 | 是 |
| 来源维度 | 单选 | 3 | 框架分析维度 | 根据选择框架动态生成 | 是 |
...
| 状态 | 单选 | 3 | 任务状态 | 未开始/进行中/已完成/已取消 | 是 |
| 飞书任务ID | 文本 | 1 | 关联飞书 Task | 飞书任务唯一标识 | 否 |
...
| 日志类型 | 单选 | 3 | 日志分类 | 进展更新/问题记录/决策记录 | 是 |
