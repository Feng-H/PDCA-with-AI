...
| 平台适配器回调能力 | 检测 `clarify` 工具是否可用 | 可用 = 原生交互 |
| `msg_type=interactive` 支持度 | 检测飞书/Slack 是否支持卡片消息 | 支持 = Interactive Card |

#### 交互模式优先级（从高到低）

| 优先级 | 模式 | 触发条件 | 用户体验 |
|--------|------|---------|---------|
| **P0** | 原生交互组件 | `clarify` 工具可用（CLI / 部分 Web 平台） | ⭐⭐⭐ |
| **P1** | Interactive Card | 飞书/Slack 且平台支持发送卡片消息 | ⭐⭐⭐ |
| **P2** | 富文本编号列表 | 所有平台（通用 fallback） | ⭐⭐ |

#### 各平台推荐方案

**飞书（Feishu / Lark）**：
- ✅ 首选：发送 Interactive Card（按钮/下拉选择），用户点击直接提交
- 🔄 备选：使用富文本 Post 格式呈现编号选项，用户输入编号回复
- 详细的卡片 JSON 模板见 [feishu-interaction.md](assets/references/feishu-interaction.md)

**CLI（Hermes / OpenClaw）**：
- ✅ 首选：使用 `clarify` 工具（原生支持多选/单选/自定义）
- 🔄 备选：文本编号列表

**其他平台（Telegram / Discord / Slack）**：
- 使用平台原生交互组件（Inline Keyboard / Block Kit 等）
- 不支持时 fallback 到富文本编号列表
...
- 所有平台一律用纯文本长问句 → 没有检测平台能力
- 飞书上用 `clarify` 工具 → 该工具在网关模式下不可用
