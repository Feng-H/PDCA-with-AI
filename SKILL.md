...
⚠️ **飞书后端为首选**：当 Agent 运行在支持飞书工具的环境中（如 OpenClaw + openclaw-lark），**必须使用飞书作为项目文档的事实来源**，包括：
- 飞书 Wiki：项目文档存储与协作
- 飞书 Bitable：项目看板与数据管理
- 飞书 Calendar：里程碑与日程
- 飞书 Task：待办事项追踪

**禁止行为**：在有飞书环境的情况下，仅将项目文档创建到本地文件系统。飞书创建失败时应重试并报告错误，不得降级为本地文件。
...
7. **项目文档必须创建到飞书 Wiki**（在有飞书环境时），本地文件仅作为无飞书时的 fallback
